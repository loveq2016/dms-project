...
public class RepostOrderReportController {
	...
	public JsonPage<Map<String,Object>> paging(PageRequest pageRequest,HttpServletRequest request,Model model) {
		...
		try {
			...
			if(null!=sysUser){
				...
				return repostService.orderReportPaging(pageRequest,filters);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void down(PageRequest pageRequest, HttpServletRequest request,
		   ...
		   pageRequest.setOrderBy("order_date");
		   ...
		   List<Map<String,Object>> list =new ArrayList<Map<String,Object>>();
			try {
				...
				if(null!=sysUser){
					...
					list = repostService.orderReportPaging(pageRequest,filters).getRows();
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
		   String classPath = new File(getClass().getResource("/excel").getFile()).getCanonicalPath(); 
		   String excelPath = classPath + File.separator + "OrderReport.xls";
		...
		try {
	        ...
	        for (Map<String,Object> map : list) {
				...
				for (int j = 0; j < 10; j++) {
                    ...
                    switch (j) {
						//代理商名称  代理商代码  区域    订单号  订单类型   订单日期   型号  订货数量 金额 备注
						...
						case 3:cellN.setCellValue(valueOf(map.get("order_code")));break;
						case 4:cellN.setCellValue(valueOf(map.get("type")));break;
						case 5:cellN.setCellValue(valueOf(map.get("order_date")));break;
						case 6:cellN.setCellValue(valueOf(map.get("models")));break;
						case 7:cellN.setCellValue(valueOf(map.get("order_number_sum")));break;
						case 8:cellN.setCellValue(valueOf(map.get("order_money_sum")));break;
						case 9:cellN.setCellValue(valueOf(map.get("remark")));break;
						default:break;
					}
				}
				row++;
