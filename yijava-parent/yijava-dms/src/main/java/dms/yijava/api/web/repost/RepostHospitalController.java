...
@Controller
@RequestMapping("/api/repostHospital")
public class RepostHospitalController {
	...
	public void down(PageRequest pageRequest, HttpServletRequest request,
		   ...
		   List<PropertyFilter> filters = PropertyFilters.build(request);
		   List<Hospital> list = hospitalService.paging(pageRequest,filters).getRows();
		   //String [] title = new String[]{"医院中文名称","医院等级","医院省份","医院县市（区）","医院地区","邮编","地址","医院电话","床位数"};
		   String classPath = new File(getClass().getResource("/excel").getFile()).getCanonicalPath(); 
		   String excelPath = classPath + File.separator + "HospitalReport.xls";
		   excel(excelPath, list, response);
	}
	
	
	
	private void excel(String excelPath,List<Hospital> list,HttpServletResponse response){
		try {
	       ...
	       */
	        ...
	        for (Hospital hospital : list) {
				...
				for (int j = 0; j < 9; j++) {
                    ...
                    switch (j) {
						case 0:cellN.setCellValue(hospital.getHospital_name());break;
						case 1:cellN.setCellValue(hospital.getLevel_name());break;
						case 2:cellN.setCellValue(hospital.getProvinces_name());break;
						case 3:cellN.setCellValue(hospital.getCity_name());break;
						case 4:cellN.setCellValue(hospital.getArea_name());break;
						case 5:cellN.setCellValue(hospital.getPostcode());break;
						case 6:cellN.setCellValue(hospital.getAddress());break;
						case 7:cellN.setCellValue(hospital.getPhone());break;
						case 8:cellN.setCellValue(hospital.getBeds());break;
						default:break;
					}
				}
				row++;
